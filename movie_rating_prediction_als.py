# Databricks notebook source exported at Thu, 15 Sep 2016 18:22:09 UTC
# MAGIC %md
...
movie_names_df = movie_ids_with_avg_ratings_df.sort(movie_ids_with_avg_ratings_df.average.desc())
movie_names_with_avg_ratings_df = movie_names_df.join(movies_df, movie_names_df.movieId == movies_df.ID, 'inner')
...
avg_rating_df = training_df.groupBy().avg()
#avg_rating_df.show(3)

# Extract the average rating value. (This is row 0, column 0.)
training_avg_rating = avg_rating_df.collect()[0][2]
...
my_rated_movies = [(my_user_id, 318, 5),(my_user_id, 858, 5),(my_user_id, 50, 5),(my_user_id, 527, 5),(my_user_id, 1221, 5),(my_user_id, 7502, 5),(my_user_id, 2959, 5),(my_user_id, 77658, 5),(my_user_id, 58559, 5),(my_user_id, 4226, 5)]

#display(my_rated_movies)
...
training_with_my_ratings_df = unionAll(training_df,my_ratings_df)
...
my_unrated_movies_df = not_rated_df.withColumn('userId',F.lit(my_user_id))
my_unrated_movies_df = reduce(lambda my_unrated_movies_df, idx: my_unrated_movies_df.withColumnRenamed(oldColumns[idx], newColumns[idx]), xrange(len(oldColumns)), my_unrated_movies_df)
...
predicted_ratings_df.show(2)
movie_names_with_avg_ratings_df.show(2)
predicted_with_counts_df = unionAll(predicted_ratings_df,movie_names_with_avg_ratings_df.drop('ID'))

#predicted_highest_rated_movies_df = predicted_with_counts_df.groupBy().counts()
